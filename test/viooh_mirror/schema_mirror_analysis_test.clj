...
(fact
 "analyse-compatibility: when the subject compatibility level isn't
   specified the global registry compatibility is used instead."
...
(fact
 "analyse-compatibility: destination schema registry information are
...
(fact
 "analyse-compatibility: test is `true` if and only if both src and
 dst compatibility levels are the same"
...
(fact
 "analyse-strict-schema-versions: If schemas match in number order and
 content test should be true"
...
(fact
 "analyse-strict-schema-versions: If schemas mismatch: 1 missing in
 destination (in this case it could be repaired by adding the missing
 schema)"
...
(fact
 "analyse-strict-schema-versions: If schemas mismatch: earlier version
 is missing (this cannot be repaired)"
...
(fact
 "analyse-schema-versions-lenient: If source and destination match
 exactly, then the test should be true"
...
(fact
 "analyse-schema-versions-lenient: If all the source schemas are in
...
(fact
 "analyse-schema-versions-lenient: If all the source schemas are in
  the destination as well (even if destination has more schemas) BUT
...
(fact
 "analyse-schema-versions-lenient: If one of more schemas are missing
 from the destination then test should be false and the missing schema
 should be listed. (this can be repaired by adding the new schema in
 the destination)"
...
(fact
 "analyse-schema-versions-lenient: If not all the source schemas are
  in the destination (even if destination has more schemas) BUT NOT IN
  THE SAME ORDER, then the test should be false (this cannot be
  repaired if compatibility level is different than NONE)"
...
(fact
 "analyse-schema-versions-lenient-unordered: If source and destination
...
(fact
 "analyse-schema-versions-lenient-unordered: If all the source schemas are in
  the destination as well (even if destination has more schemas) AND
  IN THE SAME RELATIVE ORDER, then the test should be true"
...
(fact
 "analyse-schema-versions-lenient-unordered: If all the source schemas
  are in the destination as well (even if destination has more
...
(fact
 "analyse-schema-versions-lenient-unordered: If one of more schemas
 are missing from the destination then test should be false and the
...
(fact
 "analyse-schema-versions-lenient-unordered: If not all the source
  schemas are in the destination (even if destination has more
  schemas) BUT NOT IN THE SAME ORDER, then the test should be
